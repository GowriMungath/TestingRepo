...
class TestRunModule(unittest.TestCase):
    def _import_run_with_args(self, argv):
        # Ensure we re-execute run.py each time
        ...
        # Patches are no longer active on the module attributes,
        # but the MagicMocks keep their call history.
        return (
            run_module,
            mock_overwrite,
            MockExample,
    ...
    def test_parse_args_with_required_and_optional_parameters(self):
        ...
        # Import run with controlled environment so top-level code doesn't explode
        run_module, _, _, _, _, _ = self._import_run_with_args(argv)

        # Call parse_args explicitly again to assert values
        with patch("sys.argv", argv):
    ...
    def test_feature_0_triggers_example_analysis(self):
        ...
        # ExampleAnalysis should be instantiated and .run() called
        MockExample.assert_called_once()
        MockExample.return_value.run.assert_called_once()

        # Other analysis classes should NOT be instantiated
    ...
    def test_invalid_feature_prints_message(self):
        ...
        # We also patch print to capture the message
        if "run" in sys.modules:
            del sys.modules["run"]
        ...
        # No analysis classes should be used
        MockExample.assert_not_called()
        MockContributor.assert_not_called()
        MockReopened.assert_not_called()
        MockTrend.assert_not_called()

        # Error message should have been printed once
