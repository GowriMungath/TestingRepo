...
class TestConfig(unittest.TestCase):
    ...
    def setUp(self):
        # Reset config and environment variables before each test
        config._config = None
    ...
    def test_overwrite_from_args_iteritems_fallback(self):
        class Args:
            def __init__(self):
                ...
                self.b = "hi"
        args = Args()
        # Patch vars(args).iteritems to throw AttributeError, so fallback to items
        config.overwrite_from_args(args)
        self.assertEqual(os.environ["a"], "json:1")
        self.assertEqual(os.environ["b"], "hi")
