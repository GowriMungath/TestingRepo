...
class TestUser(unittest.TestCase):
    def test_user_basic_fields_and_default_site_admin(self):
        data = {
            ...
            "login": "alice",
            "type": "User",
            # site_admin intentionally omitted to test default = False
        }
        user = mod.User(data)
...
class TestComment(unittest.TestCase):
    def test_comment_with_nested_author_and_valid_date(self):
        ...
        # basic sanity check on parsed date
    ...
    def test_comment_with_flat_author_and_invalid_date_and_comment_field(self):
        data = {
            ...
            "issue_id": 20,
            "event_id": 200,
            # 'author' is a non-dict value so it triggers the else-branch
            "author": "flat-author",
            "author_id": 99,
            "comment": "Fallback comment",
            # invalid created_at should be swallowed, leaving created_date = None
            "created_at": "not-a-date",
        }
        comment = mod.Comment(data)
        # Now we hit the else-branch and author_id should come from author_id
...
class TestEvent(unittest.TestCase):
    ...
    def test_event_with_author_fallback_and_event_type_field(self):
        data = {
            "id": 22,
            # no 'event', but 'event_type' should be used
            "event_type": "reopened",
            "issue_id": 202,
            # no 'actor', but 'author' is present
    ...
    def test_event_with_flat_author_and_invalid_date(self):
        ...
        event = mod.Event(data)
        # Else-branch: author_id from author_id, author_login None
        self.assertEqual(event.author_id, 123)
        self.assertIsNone(event.author_login)
        # invalid date should be swallowed
        self.assertIsNone(event.event_date)
...
class TestIssue(unittest.TestCase):
    def test_issue_with_nested_user_and_valid_dates_and_children(self):
        data = {
            ...
            "labels": [
                {"name": "bug"},
                "help wanted",
                123,  # unsupported type, should become Label with name=None
            ],
        }
        ...
        # the int label becomes a Label with name = None
    ...
    def test_issue_with_string_creator_and_created_date_field(self):
        data = {
            ...
            # invalid updated_date should be swallowed
        ...
        # created_date is parsed, updated_date fails to parse
        self.assertIsInstance(issue.created_date, datetime)
    ...
    def test_issue_with_missing_creator_and_no_children(self):
        data = {
            ...
            # no user/creator
            # no comments, no events, no labels
        }
        issue = mod.Issue(data)

        # Creator info should all be None
        self.assertIsNone(issue.creator_id)
...
class TestAllExport(unittest.TestCase):
    def test_all_contains_expected_symbols(self):
        # simple sanity check that __all__ is set as intended
        expected = {
            "Issue",
            "User",
            "Comment",
